...
SetPlayerWantedLevelEx(playerid, level)
{
	...
	mysql_format(Database, query, sizeof(query), "UPDATE users SET wanted = %i WHERE acc_id = %i", PlayerInfo[playerid][pWanted], PlayerInfo[playerid][pID]);
...
IncreaseWantedLevel(playerid)
{
	...
	if(PlayerInfo[playerid][pWanted] == 12)
	{
		SendClientMessage(playerid, COLOR_LIME, "You have maxed wanted levels, the police have been notified of your location");
	//	SendPoliceMessage(COLOR_BLUE, "MOST WANTED %s has been last spotted at %s", GetPlayerNameEx(playerid));
	}
	else
	{
	...
	if(PlayerInfo[playerid][pWanted] == 1 || PlayerInfo[playerid][pWanted] == 2)
	{
		SetPlayerWantedLevel(playerid, 1);
	}

	if(PlayerInfo[playerid][pWanted] == 3 || PlayerInfo[playerid][pWanted] == 4)
	{
		SetPlayerWantedLevel(playerid, 2);
	}

	if(PlayerInfo[playerid][pWanted] == 5 || PlayerInfo[playerid][pWanted] == 6)
	{
		SetPlayerWantedLevel(playerid, 3);
	}

	if(PlayerInfo[playerid][pWanted] == 7 || PlayerInfo[playerid][pWanted] == 8)
	{
		SetPlayerWantedLevel(playerid, 4);
	}

	if(PlayerInfo[playerid][pWanted] == 9 || PlayerInfo[playerid][pWanted] == 10)
	{
		SetPlayerWantedLevel(playerid, 5);
	}

	if(PlayerInfo[playerid][pWanted] == 11 || PlayerInfo[playerid][pWanted] == 12)
	{
		SetPlayerWantedLevel(playerid, 6);
	}
	return 1;
...
GetStaffRank(playerid)
{
	...
	switch(PlayerInfo[playerid][pAdmin])
	{
		...
		case 5: string = "Community Owner";
	}

	return string;
}

GetStaffRankShort(playerid)
{
	new string[24];
	...
	{
	    case 1: string = "Mod";
	    case 2: string = "Jnr. Admin";
	    case 3: string = "Snr. Admin";
	    case 4: string = "Svr. Manager";
		case 5: string = "Comm. Owner";
	}
