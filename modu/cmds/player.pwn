CMD:chatmode(playerid, params[])
{
	...
	{
	mysql_format(Database, query, sizeof(query), "UPDATE users SET chatmode = 0 WHERE acc_id = %i", PlayerInfo[playerid][pID]);
	...
	{
	mysql_format(Database, query, sizeof(query), "UPDATE users SET chatmode = 1 WHERE acc_id = %i", PlayerInfo[playerid][pID]);
	mysql_tquery(Database, query);
...
/*	else if(!strcmp(params, "gmx", true))
    ...
    {
       SendClientMessage(playerid, COLOR_ERROR, "/chatmode [public | local]");
    }
	return 1;
}
...
//	format(string7, sizeof(string7), "%s{FFFFFF}Hitmarker%s\n", string6, hits);
	ShowPlayerDialog(playerid, DIALOG_NONE, DIALOG_STYLE_MSGBOX, "Statistics", string4, "Close", "");
	return 1;
}
...
CMD:p(playerid, params[])
{
	if(isnull(params))
	{
	    return SendClientMessage(playerid, COLOR_ERROR, "Usage: /p [text]");
	}
...
CMD:ask(playerid, params[])
{
	if(isnull(params))
	{
	    return SendClientMessage(playerid, COLOR_ERROR, "[Usage]{FFFFFF}: /ask [text]");
	}
