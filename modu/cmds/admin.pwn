...
CMD:makeadmin(playerid, params[])
{
	...
	if(sscanf(params, "ui", playerb, adminlvl)) return SendClientMessage(playerid, COLOR_ERROR, "Usage: /makeadmin [playerid/name] [level 0-3]"), 1;
        
    if(!IsPlayerConnected(playerb)) return SendClientMessage(playerid, COLOR_ERROR,"Error: Player hasn't connected to the server yet."), 1;

    if(adminlvl < 0 || adminlvl > 5) return SendClientMessage(playerid, COLOR_ERROR, "Usage: Invalid Admin Level"), 1;
    ...
    mysql_format(Database, query, sizeof(query), "UPDATE `users` SET `admin` = '%i' WHERE `acc_id` = '%i'", adminlvl, PlayerInfo[playerb][pID]);
